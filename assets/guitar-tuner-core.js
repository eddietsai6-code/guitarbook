// Adapted from eddietsai6-code/twelve-tone-tuner (MIT). Trimmed to 12-TET and guitar modes.
...
export function frequencyToNote(frequency, a4 = DEFAULT_A4) {
  ...
  return { name, octave, midi, cents, frequency, targetFrequency };
}
...
export function getTuningHint(modeId) {
  ...
  if (!mode || !mode.strings.length) return "Any 12-TET note";
...
export function findClosestInstrumentTarget(frequency, modeId, a4 = DEFAULT_A4) {
  const mode = TUNING_MODES[modeId];
  if (!mode || !mode.strings.length || !Number.isFinite(frequency) || frequency <= 0) {
    return null;
  }
...
export function detectPitchAutoCorrelate(
  ...
  if (getRms(buffer) < rmsThreshold) return null;
  ...
  const maxLag = Math.min(Math.ceil(sampleRate / minFrequency), Math.floor(buffer.length / 2));
  let bestLag = -1;
  ...
  for (let lag = minLag; lag <= maxLag; lag += 1) {
    ...
    const normalized = energyA && energyB ? correlation / Math.sqrt(energyA * energyB) : 0;
  ...
  if (bestLag < 0 || bestCorrelation < correlationThreshold) return null;

  let selectedLag = bestLag;
  const significantCorrelation = Math.max(correlationThreshold, bestCorrelation * 0.82);

  for (let lag = minLag + 1; lag < maxLag; lag += 1) {
    const previous = correlations[lag - 1];
    const current = correlations[lag];
    const next = correlations[lag + 1];
    if (current >= significantCorrelation && current >= previous && current > next) {
      selectedLag = lag;
      break;
    }
  }
